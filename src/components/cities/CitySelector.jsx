...
function CitySelector({chosenCity}) {  
  ...
  const [chosenCityData, setChosenCityData] = useState(citiesData[0])
    
  function handleCityChange(event) {
    const selectedCity = citiesData.find(city => city.name === event.target.value)
    chosenCity(selectedCity)
    setChosenCityData(selectedCity)
  }  

  return ( 
  <form className={styles.selectorForm}>
    <h1>Choose a city</h1>
    <select className={styles.select} name="" id="" onChange={handleCityChange}>
